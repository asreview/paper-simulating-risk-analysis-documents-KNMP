...
def sample_prior_knowledge(df, n, seed=None):
    ...
    df_incl = df[(df["final_included"] == 1)]
    df_excl = df[(df["final_included"] == 0)]


    result = pd.DataFrame({
        "inclusion": np.random.choice(df_incl.index, n, replace=True),
        "exclusion": np.random.choice(df_excl.index, n, replace=True)
    })
