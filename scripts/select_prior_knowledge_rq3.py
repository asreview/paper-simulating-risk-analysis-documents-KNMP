...
def sample_prior_knowledge(df, n, seed=None):
    ...
    df_incl = df[((df["dataset"] == "morfine") & (df["final_included"] == 1))]
    df_excl = df[((df["dataset"] == "morfine") & (df["final_included"] == 0))]

    # to do: add prior knowledge from fentanyl (new dataset)

    result = pd.DataFrame({
         "priors" : (df_incl.index | df_excl.index)
     })
